...
const LEVEL = object({
    ...
    /**
     * Насколько близко к последней добыче падает то, что обнуляет серию.
     *
     * Ноль — мухомор появляется где придётся, единица — точно над тем местом,
     * где только что вышел фрукт. Это и есть настоящая сложность: не «успеть
     * добежать», а «взять это, не задев то». Скорость корзины тут ни при чём —
     * за неё отвечает рука игрока.
     */
    dangerProximity: number({ min: 0, max: 1 }),
    spawn: SPAWN,
});
