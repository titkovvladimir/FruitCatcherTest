...
export class LevelSession {
    ...
    /**
     * Предмет улетел мимо корзины.
     *
     * Промахом считается всё, кроме упущенного мухомора: его игрок обошёл
     * нарочно, и записывать это в ошибки — врать в лицо. Упущенное кислое
     * ошибка настоящая: серия обнуляется целиком, тогда как пойманное кислое
     * стоит одной ступени. Отсюда и правило игрока — кислое надо ловить,
     * морщась.
     *
     * Жизнь при этом дороже серии: мухомор бесплатно упустить можно, потому
     * что поймать его стоит жизни, а не ступени.
     */
    applyMiss(item: FallingItemConfig): void {
        if (this._state !== 'running' || item.combo === 'reset') {
            return;
        }
        this._missed += 1;
        this.setStreak(0);
